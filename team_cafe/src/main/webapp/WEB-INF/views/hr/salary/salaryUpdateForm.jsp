...
<!DOCTYPE html>
  <head>
	...
	function updateSalary(){
		var timeP= /^\d{1,2}$/;
		if (document.updateSalaryForm.salary_date.value == "") {
			alert("월급지급일을 입력해주세요");
		}else if (!document.updateSalaryForm.salary_date.value == timeP.test(document.updateSalaryForm.salary_date.value)) {
			alert("월급지급일을 제대로 입력해주세요");
			document.updateSalaryForm.salary_date.focus();
		}else {
			document.updateSalaryForm.submit(); //전송
		}
	}
    </script>
  </head>
   ...
   <div class="modal fade" id="addSalaryModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
          ...
          <form name="addSalaryForm" action="insert.cafe" method="post" onsubmit="return addSalary();" >
            <div class="modal-body">
              <div class="form-group">
                <label>직원이름</label>
